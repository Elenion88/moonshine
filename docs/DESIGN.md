# Moonshine - design notes

Why this project is shaped the way it is: the failures that produced each
piece, the things that looked like the problem and were not, and the dead ends
worth not re-walking.

Addresses, network names and account names throughout are placeholders. The
output they appear in is real.

For what Moonshine is and how to install it, see the [README](../README.md).
...
onto `Guest-2.4`, a guest SSID with AP/client isolation enabled. It could reach
...
remote desktop tool just feels bad. After rejoining the main `Home`
...
non-interactive path is an MDM profile pushed by IT, which for this MDM-managed
...
each registering its own node - `macbook` (100.101.10.22) and `macbook-air`
(100.101.10.23) - so the same laptop showed up twice in every menu. Both resolved
to the same LAN address, `192.168.1.50`, on different ports.
...
```bash
moonshine connect tower                              # desktop session
moonshine connect tower -p gaming --overlay          # game, with stats
moonshine connect tower "Steam Big Picture" -p gaming
```
...
|---|---|---|
| ICMP through the tunnel (`100.101.10.24`) | **4.33 ms** | 2-9 ms |
| ICMP over the LAN (`192.168.1.51`) | **0 ms** | none |
...
```
macbook-air   active; direct 192.168.1.50:41641
                            ^^^^^^^^^^^^^ this
```
...
- **Pairing is by server identity, not address.** `moonlight list 192.168.1.52`
  returns the app list on a host paired as `tower`, so no re-pairing.
...
Note this Mac is MDM-enrolled, so a PPPC profile could block that
permission outright. If the toggle won't stick, that's why.

## Reference setup

**Host** (Windows 11, Ryzen 5 9600X, RTX 3090, `100.101.10.11`)
...
- Web UI: <https://localhost:47990> (user `<your-sunshine-user>`)

**Client** (macOS 26.5.2, arm64, 2560x1664, `100.101.10.22`)
...
- `macbook-air` (`100.101.10.23`) resolves to the same LAN address as `macbook`
