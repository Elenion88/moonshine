...
- **`tray_macos.py`** - macOS menu bar item

### Why there is a window as well as a menu bar item
...
rendered anything. No fix to this code could have helped, so the window exists as
the reliable entry point. The menu bar agent still runs and will appear on its
own whenever space frees up - plugging in an external display, for instance.
...
The macOS title uses **text glyphs, not emoji**. Emoji have fixed colours, so the
original white circle for the idle state was invisible against a light menu bar -
the app was running correctly and simply could not be seen. Text glyphs inherit
the menu bar's own colour and render in both themes. Only genuine problem states
use a coloured emoji, so the bar stays quiet when everything is fine.

The macOS app is also a real `.app` bundle (`scripts/build_macos_app.sh`) with
`LSUIElement`, rather than a bare script under launchd. Note `~/Applications` on
this Mac is **owned by root** courtesy of MDM, so the bundle is built into
`~/.local/share/moonshine/` instead.

If Ice is running, check its hidden section - `AutoRehide` re-hides items every
15 seconds. Drag `Moonshine` into Ice's Always Visible section to pin it.

Autostart is a checkbox in the menu, or from a terminal:
...
# macOS    (LaunchAgent)
~/.local/share/moonshine/venv/bin/python ~/.local/share/moonshine/tray_macos.py --install-autostart
```
...
tray_macos.py      macOS menu bar app (rumps, runs from ./venv)
...
label, a registry value, two icon filenames and a state directory. It lives in
`brand.py` now, so the next change is one line.
