...
jitter, and a button per profile. Windows: Start Menu → **Moonshine**. macOS:
**Moonshine.app**.

**The tray** (Windows only) carries the status colour and the same menu.
There is no macOS menu bar item, for a reason worth reading:
[design notes](docs/DESIGN.md#why-there-is-no-macos-menu-bar-item).
...
moonshine setup                   # configure this machine as a host
moonshine display                 # host displays and the stream's fps ceiling
moonshine logs [--show]           # recorded sessions
moonshine hide / unhide <host>    # drop a host from listings and the menu
```
...
packaging/         Build specs, the installer, and what is still missing
